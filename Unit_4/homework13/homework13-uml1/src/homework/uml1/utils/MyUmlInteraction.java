...
import homework.uml1.elements.classifier.MyUmlClass;
import homework.uml1.elements.classifier.MyUmlClassifier;
import homework.uml1.elements.classifier.MyUmlInterface;
import homework.uml1.elements.meta.MyUmlOperation;
import homework.uml1.elements.meta.MyUmlParameter;
...
public class MyUmlInteraction implements UmlInteraction {
    ...
    public MyUmlInteraction(UmlElement... elements) {
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            switch (elementType) {
                case UML_CLASS:
                    addUmlClass((UmlClass) element);
                    break;
                case UML_INTERFACE:
                    addUmlInterface((UmlInterface) element);
                    break;
                default:
                    break;
            }
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            switch (elementType) {
                case UML_ATTRIBUTE:
                    addUmlAttribute((UmlAttribute) element);
                    break;
                case UML_OPERATION:
                    addUmlOperation((UmlOperation) element);
                    break;
                case UML_ASSOCIATION_END:
                    addUmlAssociationEnd((UmlAssociationEnd) element);
                    break;
                default:
                    break;
            }
        ...
        for (int i = 0; i < elements.length; i++) {
            ...
            switch (elementType) {
                case UML_PARAMETER:
                    addUmlParameter((UmlParameter) element);
                    break;
                case UML_ASSOCIATION:
                    addUmlAssociation((UmlAssociation) element);
                    break;
                case UML_GENERALIZATION:
                    addUmlGeneralization((UmlGeneralization) element);
                    break;
                case UML_INTERFACE_REALIZATION:
                    addUmlInterfaceRealization(
    ...
    private void addUmlClass(UmlClass umlClass) {
    ...
    private void addUmlInterface(UmlInterface umlInterface) {
    ...
    private void addUmlAttribute(UmlAttribute umlAttribute) {
        MyUmlAttribute myUmlAttribute = new MyUmlAttribute(umlAttribute);
        // 映射到所在类或接口
        String clsfrId = myUmlAttribute.getClassId();
    ...
    private void addUmlOperation(UmlOperation umlOperation) {
        ...
        // 映射到所在类或接口
        String clsfrId = myUmlOperation.getClassId();
        if (id2Class.containsKey(clsfrId)) {
            MyUmlClass myUmlClass = id2Class.get(clsfrId);
    ...
    private void addUmlAssociationEnd(UmlAssociationEnd umlAssociationEnd) {
    ...
    private void addUmlParameter(UmlParameter umlParameter) {
        MyUmlParameter myUmlParameter = new MyUmlParameter(umlParameter);
        // 映射到所在方法
        String methodId = myUmlParameter.getClassId();
        MyUmlOperation myUmlOperation = id2Operation.get(methodId);
        myUmlOperation.setPara(myUmlParameter);
    }

    private void addUmlAssociation(UmlAssociation umlAssociation) {
    ...
    private void addUmlGeneralization(UmlGeneralization umlGeneralization) {
    ...
    private void addUmlInterfaceRealization(
